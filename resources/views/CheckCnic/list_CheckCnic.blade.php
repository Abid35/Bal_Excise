...
@section('content')
  ...
  <div id="maindiv" class="container main">
    ...
    <div class="row">
      ...
      <div class="col-md-8" style="margin-top: 1%">
        <form action="{{ url('SearchCnic') }}" method="post">
          @csrf
          <div class="form-group has-search">
            <span class="fa fa-search form-control-feedback"></span>
            <input type="text" class="form-control" name="search"
            @if(isset($ser)) value="{{ $ser }}" @endif
            placeholder="CNIC No. / NTN No.">
          </div>
        </form>
      </div>
          ...
          @if(isset($data))
            @foreach($data as $x)
              @if($x->vehicle_status==0)
                <tr style="background-color: #FBEC5D;">
  ...
  <script>
    document.getElementById('exportBtn').addEventListener('click', function() {
      ...
      link.href = url;
      link.setAttribute('download', 'vehicles_cnic_data.xls');
      link.click();
    });
  </script>
@endsection
